...
class EpsonEscpService
{
    ...
    /**
     * Membuat faktur teks khusus 80 kolom untuk printer dot-matrix Epson LQ.
     * Menggunakan header minimal dan tidak menampilkan informasi rekening bank.
     */
    public function generateInvoiceKhusus(array $sale, iterable $details): string
    {
        $out = [];
        ...
        // Baris ini ukurannya besar (Double Width): '39' di kiri, 'faktur / surat jalan' di kanan
        $out[] = $bigOn . $this->pad('39', 32) . $this->pad('faktur / surat jalan', 34, ' ', STR_PAD_LEFT) . $bigOff;
        
        // Baris di bawahnya berisi nomor invoice tanpa alamat toko
        $out[] = $this->pad('', 70) . $this->pad('no: ' . ($sale['no_invoice'] ?? '-'), 62, ' ', STR_PAD_LEFT);
        $out[] = '';
        ...
        $alamat = $sale['alamat_pelanggan'] ?? '-';
        $tgl = !empty($sale['tgl_transaksi']) ? date('d-m-Y', strtotime((string) $sale['tgl_transaksi'])) : '-';
        $out[] = '│' . $this->pad(' Alamat : ' . $alamat, 64) . '│' . $this->pad(' Tanggal    : ' . $tgl, 65) . '│';
        $out[] = '└' . str_repeat('─', 64) . '┴' . str_repeat('─', 65) . '┘';
        ...
        $out[] = '│' . 
            $this->pad(' No', 4) . '│' . 
            $this->pad(' Nama Barang', 67) . '│' . 
            $this->pad('Qty', 8, ' ', STR_PAD_BOTH) . '│' . 
            $this->pad(' Satuan', 10) . '│' . 
            $this->pad('Harga', 17, ' ', STR_PAD_BOTH) . '│' . 
            $this->pad('Jumlah', 19, ' ', STR_PAD_BOTH) . '│';
        $out[] = '├' . str_repeat('─', 4) . '┼' . str_repeat('─', 67) . '┼' . str_repeat('─', 8) . '┼' . str_repeat('─', 10) . '┼' . str_repeat('─', 17) . '┼' . str_repeat('─', 19) . '┤';
        ...
        foreach ($details as $detail) {
            ...
            $out[] = '│' . 
                     $this->pad(' ' . $number++, 4) . '│' . 
                     $this->pad(' ' . $name, 67) . '│' . 
                     $this->pad(' ' . $this->formatQty($qty), 8, ' ', STR_PAD_RIGHT) . '│' . 
                     $this->pad(' ' . $unit, 10) . '│' . 
                     $this->pad(' ' . number_format($price, 0, ',', '.'), 17, ' ', STR_PAD_RIGHT) . '│' . 
                     $this->pad(' ' . number_format($subtotal, 0, ',', '.'), 19, ' ', STR_PAD_RIGHT) . '│';
        ...
        // Garis batas sebelum TOTAL: Gabungan kolom 1-5 sepanjang 110 karakter
        $out[] = '├' . str_repeat('─', 110) . '┼' . str_repeat('─', 19) . '┤';
        $out[] = '│' . $this->pad('GRAND TOTAL', 110, ' ', STR_PAD_BOTH) . '│' . $this->pad(' Rp ' . number_format($netTotal, 0, ',', '.'), 19, ' ', STR_PAD_RIGHT) . '│';
        $out[] = '└' . str_repeat('─', 110) . '┴' . str_repeat('─', 19) . '┘';
